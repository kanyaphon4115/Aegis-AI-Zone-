...
function mockAI({ system = "", messages = [] }) {
  ...
  // Preserve every endpoint's existing JSON contract. This is deliberately
  // local and deterministic: no API key, network request, or image storage is
  // required for a scan to complete in development/testing.
  if (text.includes("อ่านภาพกราฟ")) return JSON.stringify({
    b: "WAIT", c: 45, p: 0, e: 0, sl: 0, tp: 0, et: "wait", w: [], ps: 0.01,
    s: "XAUUSD", tf: "M5", ed: "Mock AI ภายในระบบ", en: "รอราคายืนยันก่อนเข้า",
    sw: "รอจุดเข้า", tw: "รอจุดเข้า", lr: "", st: "รอการยืนยัน",
    r: ["โหมดทดสอบภายใน", "ยังไม่อ่านแท่งเทียนจริง", "รอยืนยัน price action"],
    iv: "ใช้สำหรับทดสอบระบบเท่านั้น", rk: "Mock AI ไม่ใช่คำแนะนำการลงทุน",
  });
  if (text.includes("แนวโน้มระยะสั้น")) return JSON.stringify({
    spot: "", change: "", dir: "flat", bias: "neutral", score: 0, up: 50,
    horizon: "โหมด Mock", summary: "ข้อมูลจำลองภายในระบบ ไม่มีข้อมูลตลาดจริง", support: [], resistance: [], drivers: [],
  });
...
const H = {
  ...
  "POST /api/scan": async (req, res) => {
    ...
    console.info("[scan] request", {
      endpoint: "/api/scan", contentType: req.headers["content-type"] || "",
      transport: upload ? "multipart" : "json", mime: mime || "",
      bytes: upload?.size || (typeof image === "string" ? Buffer.byteLength(image, "base64") : 0),
      bodyKeys: Object.keys(req.body || {}).filter((key) => key !== "image"),
    });
    ...
    try {
      ...
      console.error("[scan] analysis failed", { endpoint: "/api/scan", message: error?.message || "unknown" });
      return err(res, 502, "วิเคราะห์ไม่สำเร็จ ลองใหม่อีกครั้ง สิทธิ์ยังไม่ถูกตัด");
    }
    ...
    console.info("[scan] response", { endpoint: "/api/scan", status: 200, bias: result.b, confidence: result.c, unlimited: unl });
